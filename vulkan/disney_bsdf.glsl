...
bool same_hemisphere(in const vec3 w_o, in const vec3 w_i, in const vec3 n) {
...
float smith_shadowing_ggx(float h_dot_o, float alpha_g) {
	float a = alpha_g * alpha_g;
	float b = h_dot_o * h_dot_o;
	return 1.f / (h_dot_o + sqrt(a + b - a * b));
}

float smith_shadowing_ggx_aniso(float h_dot_o, float o_dot_x, float o_dot_y, vec2 alpha) {
...
vec3 sample_lambertian_dir(in const vec3 n, in const vec3 v_x, in const vec3 v_y, in const vec2 s) {
...
vec3 sample_gtr_1_h(in const vec3 n, in const vec3 v_x, in const vec3 v_y, float alpha, in const vec2 s) {
...
vec3 sample_gtr_2_h(in const vec3 n, in const vec3 v_x, in const vec3 v_y, float alpha, in const vec2 s) {
	float phi_h = 2.f * M_PI * s.x;
...
vec3 sample_gtr_2_aniso_h(in const vec3 n, in const vec3 v_x, in const vec3 v_y, in const vec2 alpha, in const vec2 s) {
	float x = 2.f * M_PI * s.x;
	vec3 w_h = sqrt(s.y / (1.f - s.y)) * (alpha.x * cos(x) * v_x + alpha.y * sin(x) * v_y) + n;
	return normalize(w_h);
}

float lambertian_pdf(in const vec3 w_i, in const vec3 n) {
	float d = dot(w_i, n);
	if (d > 0.f) {
		return d * M_1_PI;
	}
...
float gtr_1_pdf(in const vec3 w_o, in const vec3 w_i, in const vec3 n, float alpha) {
...
float gtr_2_pdf(in const vec3 w_o, in const vec3 w_i, in const vec3 n, float alpha) {
	if (!same_hemisphere(w_o, w_i, n)) {
		return 0.f;
	}
	vec3 w_h = normalize(w_i + w_o);
...
float gtr_2_transmission_pdf(in const vec3 w_o, in const vec3 w_i, in const vec3 n,
...
float gtr_2_aniso_pdf(in const vec3 w_o, in const vec3 w_i, in const vec3 n,
	in const vec3 v_x, in const vec3 v_y, const vec2 alpha)
{
	if (!same_hemisphere(w_o, w_i, n)) {
		return 0.f;
	}
...
vec3 disney_diffuse(in const DisneyMaterial mat, in const vec3 n,
	in const vec3 w_o, in const vec3 w_i)
{
	vec3 w_h = normalize(w_i + w_o);
	...
	return mat.base_color * M_1_PI * mix(1.f, fd90, fi) * mix(1.f, fd90, fo);
}

vec3 disney_microfacet_isotropic(in const DisneyMaterial mat, in const vec3 n,
	in const vec3 w_o, in const vec3 w_i)
{
	vec3 w_h = normalize(w_i + w_o);
	float lum = luminance(mat.base_color);
	vec3 tint = lum > 0.f ? mat.base_color / lum : vec3(1, 1, 1);
	vec3 spec = mix(mat.specular * 0.08 * mix(vec3(1, 1, 1), tint, mat.specular_tint), mat.base_color, mat.metallic);
...
vec3 disney_microfacet_transmission_isotropic(in const DisneyMaterial mat, in const vec3 n,
	in const vec3 w_o, in const vec3 w_i)
{
	...
	float g = smith_shadowing_ggx(abs(dot(w_h, w_i)), alpha) * smith_shadowing_ggx(abs(dot(w_h, w_o)), alpha);
...
vec3 disney_microfacet_anisotropic(in const DisneyMaterial mat, in const vec3 n,
	in const vec3 w_o, in const vec3 w_i, in const vec3 v_x, in const vec3 v_y)
{
	vec3 w_h = normalize(w_i + w_o);
	float lum = luminance(mat.base_color);
	vec3 tint = lum > 0.f ? mat.base_color / lum : vec3(1, 1, 1);
	...
	vec3 f = mix(spec, vec3(1, 1, 1), schlick_weight(dot(w_i, w_h)));
	float g = smith_shadowing_ggx_aniso(dot(w_h, w_i), abs(dot(w_i, v_x)), abs(dot(w_i, v_y)), alpha)
		* smith_shadowing_ggx_aniso(dot(w_h, w_o), abs(dot(w_o, v_x)), abs(dot(w_o, v_y)), alpha);
	return d * f * g;
}

float disney_clear_coat(in const DisneyMaterial mat, in const vec3 n,
	in const vec3 w_o, in const vec3 w_i)
{
	vec3 w_h = normalize(w_i + w_o);
...
vec3 disney_sheen(in const DisneyMaterial mat, in const vec3 n,
	in const vec3 w_o, in const vec3 w_i)
{
	vec3 w_h = normalize(w_i + w_o);
	float lum = luminance(mat.base_color);
...
vec3 disney_brdf(in const DisneyMaterial mat, in const vec3 n,
	in const vec3 w_o, in const vec3 w_i, in const vec3 v_x, in const vec3 v_y)
{
	if (!same_hemisphere(w_o, w_i, n)) {
...
float disney_pdf(in const DisneyMaterial mat, in const vec3 n,
	in const vec3 w_o, in const vec3 w_i, in const vec3 v_x, in const vec3 v_y)
{
	float alpha = max(0.001, mat.roughness * mat.roughness);
	float aspect = sqrt(1.f - mat.anisotropy * 0.9f);
	vec2 alpha_aniso = vec2(max(0.001, alpha / aspect), max(0.001, alpha * aspect));
 ...
 */
vec3 sample_disney_brdf(in const DisneyMaterial mat, in const vec3 n,
	in const vec3 w_o, in const vec3 v_x, in const vec3 v_y, inout PCGRand rng,
	out vec3 w_i, out float pdf)
{
	int component = 0;
	if (mat.specular_transmission == 0.f) {
		component = int(pcg32_randomf(rng) * 3.f);
		component = clamp(component, 0, 2);
	} else {
		component = int(pcg32_randomf(rng) * 4.f);
		component = clamp(component, 0, 3);
	}

	vec2 samples = vec2(pcg32_randomf(rng), pcg32_randomf(rng));
	if (component == 0) {
